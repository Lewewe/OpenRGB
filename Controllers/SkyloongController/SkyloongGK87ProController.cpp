/*---------------------------------------------------------*\
| SkyloongGK87ProController.cpp                             |
|                                                           |
|   Driver for Skyloong GK87 Pro                            |
|                                                           |
...
SkyloongGK87ProController::SkyloongGK87ProController(hid_device* dev_handle, const char* path, std::string dev_name)
{
    dev         = dev_handle;
    location    = path;
    name        = dev_name;
...
SkyloongGK87ProController::~SkyloongGK87ProController()
{
    SendCommand(command::mode, MODE_OFFLINE);
    hid_close(dev);
}

std::string SkyloongGK87ProController::GetDeviceLocation()
{
    return("HID: " + location);
}

std::string SkyloongGK87ProController::GetDeviceName()
{
    return(name);
}

void SkyloongGK87ProController::SendCommand(char command, char sub_command)
{
...
void SkyloongGK87ProController::SendColorPacket(std::vector<RGBColor> colors, std::vector<led> *leds, int brightness)
{
...
void SkyloongGK87ProController::SetLEDefine(int address, unsigned char *le_data, int le_data_length)
{
    unsigned char buf[PACKET_SIZE];
    memset(buf, 0x00, PACKET_SIZE);
...
uint16_t SkyloongGK87ProController::Crc16CcittFalse(const uint8_t *buffer, uint16_t size)
{
    uint16_t crc = 0xFFFF;
